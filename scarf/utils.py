...
def check_single_preflist(s, pair_list, num_hospital):
  assert(np.all([len(p) == 2 and p[0] == s for p in pair_list]))
  assert(pair_list[-1] == (s, num_hospital))


def check_couple_preflist(c, pair_list, num_hospital):
  assert(np.all([len(p) == 3 and p[0] == c for p in pair_list]))
  assert(pair_list[-1] == (c, num_hospital, num_hospital))
...
def recover_pref_lists(num_single, num_couple, num_hospital, U,
  ...
  for s in range(num_single):
    ...
    check_single_preflist(s, single_s_pair_list, num_hospital)
  ...
  for c in range(num_couple):
    ...
    check_couple_preflist(c, couple_c_pair_list, num_hospital)
